...
function FileBadge({ hit }: { hit: SearchHit }) { const type = getFileType(hit); return <div aria-label={`文件类型 ${type}`} className={`grid h-10 w-10 shrink-0 place-items-center rounded-lg text-[10px] font-semibold ring-1 ring-inset ${fileBadgeTone(type)}`}>{type === "FILE" ? <FileText size={17} /> : type}</div>; }
/** 文件类型徽标配色：恢复类型语义色，作为结果条目的视觉锚点 */
export function fileBadgeTone(type: string) {
  const map: Record<string, string> = {
    ...
    TXT: "bg-sky-50 text-sky-600 ring-sky-200",
  };
  return map[type] || "bg-slate-100 text-slate-600 ring-slate-200";
}
function ResultMeta({ hit }: { hit: SearchHit }) { return <div className="mt-3 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-slate-500"><span>{getFileType(hit)}</span><span aria-hidden="true" className="text-slate-300">·</span><span className="tabular">{formatDate(hit.updatedAt || hit.createdAt)} 更新</span><span aria-hidden="true" className="text-slate-300">·</span><span className="rounded-md bg-slate-100 px-1.5 py-0.5 text-slate-600">{permissionLabel(hit.permissionScope)}</span></div>; }
