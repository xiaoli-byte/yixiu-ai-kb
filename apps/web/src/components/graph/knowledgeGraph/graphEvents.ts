...
type G6EventHandler = (e: { target: G6EventTarget; targetType?: string }) => void;
...
export function bindGraphEvents(
  ...
  const handleCanvasMove: G6EventHandler = (e) => {
    if (e.targetType === "canvas") handlers.onHover(null);
  };
  ...
  graph.on("node:pointermove", handleEnter);
  graph.on("node:pointerleave", handleLeave);
  graph.on("canvas:pointermove", handleCanvasMove);
  graph.on("canvas:pointerleave", handleLeave);
  ...
  return () => {
    ...
    graph.off("node:pointermove", handleEnter);
    graph.off("node:pointerleave", handleLeave);
    graph.off("canvas:pointermove", handleCanvasMove);
    graph.off("canvas:pointerleave", handleLeave);
